...
import { Users, Image, Video, Newspaper, ArrowRight, X, ChevronLeft, ChevronRight, CalendarDays, Briefcase, Smartphone, Database, TrendingUp, Award, Clock, Download, CheckCircle2, Rocket } from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import { STATS_COUNTERS, SPEAKERS_LIST, NEWS_ARTICLES, SPONSOR_GROUPS, PUBLICATIONS_DATA, STAGE_PRESENTERS, STALL_EXHIBITORS } from "../data";
import { AGENDA_DATA, getSessionSlug, SESSION_THEMES } from "../agendaData";
...
function SponsorLogo({ sponsor }: { sponsor: { name: string; logoUrl?: string; url?: string } }) {
  ...
  const fallback = (
    <span className="font-sans font-bold text-slate-600 text-xs text-center" style={{ display: img ? "none" : "block" }}>
      {sponsor.name}
    </span>
  );
  if (!sponsor.url) return <div className={box}>{img}{fallback}</div>;
  return (
    <a
      href={sponsor.url}
      target="_blank"
...
const YOUTUBE_VIDEOS = [
  "https://www.youtube.com/watch?v=BK3W9iEJWzY",
  "https://www.youtube.com/watch?v=h9CyR_icKXA",
...
export default function HomeDetails() {
  ...
  return (
    <div id="home-details-section" className="bg-slate-50 pt-16 border-t border-slate-100">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center gap-3 mb-12">
            ...
            <p className="text-sm sm:text-base text-slate-500 font-sans text-center max-w-lg">
              One day, five themed sessions, from the inaugural keynotes to the closing note.
            </p>
          </div>
            ...
            {HOME_AGENDA_SESSIONS.map((group, idx) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex flex-col bg-white rounded-2xl border ${group.border} shadow-xs hover:shadow-md transition-all duration-300 overflow-hidden`}
                >
                  <div className={`flex flex-wrap items-center justify-between gap-2 px-5 py-3 ${group.bg} border-b ${group.border}`}>
                    <div className="flex items-center gap-3 min-w-0">
                      <IconComponent className={`w-5 h-5 shrink-0 ${group.color}`} />
                      <div className="min-w-0">
                        <h4 className="font-display font-bold text-sm sm:text-base text-slate-800 truncate">
                          {group.session}
                        </h4>
                        {SESSION_THEMES[group.session] && (
                          <p className={`text-sm font-sans font-medium mt-0.5 ${group.color}`}>
                            {SESSION_THEMES[group.session]}
                          </p>
                        )}
                      </div>
                    </div>
                    <span className={`shrink-0 text-xs font-sans font-bold ${group.color}`}>
                      {group.time}
                    </span>
                  </div>

                  <ul className="flex-1 px-5 py-4 space-y-3.5">
                    {group.items.map((item, tIdx) => (
                      <li key={tIdx} className="flex items-start gap-2.5">
                        <CheckCircle2 className={`w-4 h-4 mt-0.5 shrink-0 ${group.color}`} />
                  ...
                  <div className="flex justify-end px-5 py-2.5 bg-slate-50/60 border-t border-slate-100">
                    <span className={`inline-flex items-center gap-1 text-xs font-sans font-bold ${group.color} group-hover:translate-x-1 transition-transform duration-200`}>
                      View details
          ...
          <div className="relative w-full overflow-hidden py-4">
            ...
            <div className="flex gap-6 animate-marquee whitespace-nowrap">
              {[...SPEAKERS_LIST, ...SPEAKERS_LIST].map((spk, idx) => (
                ...
                >
                  <div className="w-32 h-32 rounded-full overflow-hidden bg-slate-100 mb-3 relative mx-auto border-2 border-slate-200 flex items-center justify-center">
                    {spk.avatarUrl ? (
                      <img
                        src={spk.avatarUrl}
                        alt={spk.name}
      ...
      <section className="w-full bg-slate-50 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-10">
              {SPONSOR_GROUPS.slice(2, 3).map((group) => (
                <div key={group.title} className="text-center">
                  <h4 className="text-xs font-sans font-bold text-slate-500 uppercase tracking-widest mb-3">
                    {group.title}
                  </h4>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-10">
              {SPONSOR_GROUPS.slice(3, 5).map((group) => (
                <div key={group.title} className="text-center">
                  <h4 className="text-xs font-sans font-bold text-slate-500 uppercase tracking-widest mb-3">
                    {group.title}
                  </h4>
            ...
            <div className="flex flex-wrap justify-center gap-6 sm:gap-8">
              {SPONSOR_GROUPS.slice(5,6).map((group) => (
                <div key={group.title} className="text-center">
                  <h4 className="text-xs font-sans font-bold text-slate-500 uppercase tracking-widest mb-3">
                    {group.title}
                  </h4>
            ...
            <div className="flex flex-wrap justify-center gap-6 sm:gap-8">
              {SPONSOR_GROUPS.slice(6).map((group) => (
                <div key={group.title} className="text-center">
                  <h4 className="text-xs font-sans font-bold text-slate-500 uppercase tracking-widest mb-3">
                    {group.title}
                  </h4>
      ...
      <section className="relative w-full bg-gradient-to-b from-[#121652] to-[#111332] py-20 text-white overflow-hidden border-t border-white/[0.04]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center gap-3 mb-12">
            <div className="flex items-center gap-2">
              <span className="p-2.5 bg-white/10 text-dnc-orange rounded-xl border border-white/10">
            ...
            <span className="w-12 h-1 rounded-full bg-dnc-orange"></span>
          </div>
        </div>

        <div className="space-y-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <p className="text-xs font-sans font-bold text-white/60 uppercase tracking-widest mb-3 text-center">
              Digital Spark
            </p>
            <div className="relative overflow-hidden">
              <div className="absolute inset-y-0 left-0 w-16 sm:w-32 bg-gradient-to-r from-[#121652] to-transparent z-10 pointer-events-none"></div>
              <div className="absolute inset-y-0 right-0 w-16 sm:w-32 bg-gradient-to-l from-[#111332] to-transparent z-10 pointer-events-none"></div>
              <div
                className="flex gap-5 animate-marquee-reverse whitespace-nowrap"
                style={{ animationDuration: `${STAGE_PRESENTERS.length * 3}s` }}
              >
                {[...STAGE_PRESENTERS, ...STAGE_PRESENTERS].map((p, idx) => (
                  <div
                    key={`spark-${idx}`}
                    className="w-36 sm:w-44 aspect-3/2 shrink-0 bg-white border border-white/10 rounded-xl flex items-center justify-center p-4"
                  >
                    {p.logo && (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative overflow-hidden">
              ...
              <div className="absolute inset-y-0 right-0 w-16 sm:w-32 bg-gradient-to-l from-[#111332] to-transparent z-10 pointer-events-none"></div>
              <div
                className="flex gap-5 animate-marquee whitespace-nowrap"
                style={{ animationDuration: `${STALL_EXHIBITORS.length * 3}s` }}
              >
                {[...STALL_EXHIBITORS, ...STALL_EXHIBITORS].map((e, idx) => (
                  <div
                    key={`stall-${idx}`}
                    className="w-36 sm:w-44 aspect-3/2 shrink-0 bg-white border border-white/10 rounded-xl flex items-center justify-center p-4"
                  >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-center mt-12">
          <Link
            to="/digital-showcase"
            className="inline-flex items-center gap-2 px-6 py-3 bg-dnc-blue text-white font-bold text-sm sm:text-base rounded-xl hover:bg-dnc-blue/90 transition-all duration-300 shadow-md hover:shadow-lg group"
          >
